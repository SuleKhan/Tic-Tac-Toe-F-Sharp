...
[<RequireQualifiedAccess>]
module Board =
    let init (dimensions: Dimensions) = {Dimensions = dimensions
                                         Spaces = Map.empty }
    
    let containsCoord (coord: Coord) (board: Board) =
        0 <= coord.Row && coord.Row < board.Dimensions.Height && 0 <= coord.Col && coord.Col < board.Dimensions.Width
...
module TicTacToeModel =
    ...
    let checkDiagsWinner (board: Board) =
        ...
        let rightDiagCoords = List.init rank (fun i -> Coord.init i (board.Dimensions.Width - i - 1))
    ...
    let private checkWinner (board: Board) =
        ...
        let rowWinner = List.init board.Dimensions.Height (flip checkRowWinner board) |> List.tryPick id
        let colWinner = List.init board.Dimensions.Width (flip checkColWinner board) |> List.tryPick id
    ...
    let updateGameState (model: TicTacToeModel) =
        ...
        | Some winner, _ -> {model with GameState = Victory winner }
    ...
    let private selectSpace (player: Player) (coord: Coord) (model: TicTacToeModel) =
        if model.Board.Spaces.ContainsKey coord then model
        else model |> setPlayerSpace player coord |> changeTurn |> updateGameState
    
    let resetGame (model: TicTacToeModel) =
        match model.NewDimensions with
        | Ok newDimensions -> {Board = Board.init newDimensions
                               NewDimensions = model.NewDimensions 
                               Turn = Player.X
                               GameState = InProgress }
        | Error _ -> model
